...
export default function AdminModeler() {
  ...
  const [hiddenWallIds, setHiddenWallIds] = useState([])
  ...
  const totalFloorArea = useMemo(() => detectedRooms.reduce((sum, room) => sum + room.area, 0), [detectedRooms])
  const exitWalk = useCallback(() => setWalkMode(false), [])
  ...
  useEffect(() => {
    ...
    for (let step = -200; step <= 200; step += 1) {
      const n=step/10;const major=step%10===0;const xa = project({ x: n, y: -20 }); const xb = project({ x: n, y: 20 }); const ya = project({ x: -20, y: n }); const yb = project({ x: 20, y: n })
    ...
    model.walls.filter((wall)=>!hiddenWallIds.includes(wall.id)).forEach((wall) => {
    ...
    model.openings.filter((opening)=>!hiddenWallIds.includes(opening.wallId)).forEach((opening) => {
      ...
      if(view==='3d'){
        ...
        if(opening.type==='door'){const swing=DOOR_SWINGS.includes(opening.swing)?opening.swing:'left-in';const hinge=swing.startsWith('right')?b:a;const hingePoint=swing.startsWith('right')?pb:pa;const direction=swing.endsWith('out')?-1:1;const leafEnd=project({x:hinge.x-dy*opening.width*direction,y:hinge.y+dx*opening.width*direction});ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(hingePoint.x,hingePoint.y);ctx.lineTo(leafEnd.x,leafEnd.y);ctx.stroke()}
      }
  ...
  function handleCanvasClick(event){
    ...
    if(tool==='select'){
      const candidates=[...detectedRooms.map((room)=>({collection:'rooms',id:room.id,distance:pointInRoom(basePoint,room)?0:Infinity})),...model.furniture.map((item)=>({collection:'furniture',id:item.id,distance:Math.hypot(screen.x-project(item).x,screen.y-project(item).y)})),...model.openings.filter((item)=>!hiddenWallIds.includes(item.wallId)).map((item)=>{const wall=model.walls.find((w)=>w.id===item.wallId);const p=wall?project(pointOnWall(wall,item.t)):screen;return{collection:'openings',id:item.id,distance:Math.hypot(screen.x-p.x,screen.y-p.y)}}),...model.walls.filter((wall)=>!hiddenWallIds.includes(wall.id)).map((wall)=>({collection:'walls',id:wall.id,distance:segmentHit(screen,project(wall.start),project(wall.end)).distance}))].sort((a,b)=>a.distance-b.distance)
    ...
    if(tool==='wall'){
      ...
      const opening={id:crypto.randomUUID(),type:tool,wallId:hit.wall.id,t:hit.t,width:Number(settings.openingWidth),height:Number(settings.openingHeight),sill:tool==='window'?Number(settings.sill):0,...(tool==='door'?{swing:'left-in'}:{})};const validation=validateOpeningPlacement(model,opening)
  ...
  function updateBuildingSetting(section,field,value){
    setModel((current)=>updateBuilding(current,section,field,value));setSaveState('Cambios sin guardar')
  }
  ...
  function updateSelectedDoorSwing(swing){
    if(selection?.collection!=='openings')return
    setModel((current)=>updateDoorSwing(current,selection.id,swing));setSaveState('Cambios sin guardar')
  }
  function updateSelectedRoom(field,value){
    if(selection?.collection!=='rooms')return
    setModel((current)=>updateRoom(current,selection.id,field,value));setSaveState('Cambios sin guardar')
  }
  function hideSelectedWall(){
    if(selection?.collection!=='walls')return
    setHiddenWallIds((items)=>items.includes(selection.id)?items:[...items,selection.id]);setSelection(null)
  }
  function showAll(){
    setHiddenWallIds([])
  ...
  function applyPendingPlan(){
    ...
    setModel((current)=>{
      ...
      for(const action of pendingPlan.actions){
        if(action.type==='clear_model'){next.walls=[];next.openings=[];next.furniture=[];next.rooms=[];continue}
  ...
  const title=selectedItem?`${selection.collection==='walls'?'Muro':selection.collection==='openings'?(selectedItem.type==='door'?'Puerta':'Ventana'):selection.collection==='rooms'?'Ambiente':FURNITURE[selectedItem.type]?.label} seleccionado`:tool==='door'?'Nueva puerta':tool==='window'?'Nueva ventana':tool==='furniture'?'Nuevo mueble':'Nuevo muro'
  ...
  return <section className="modeler-page"><header className="modeler-header"><div className="modeler-brand"><OxidaWordmark/><span>MercadoBRA Modelador</span><em>Beta privada</em></div><nav><Link to="/admin/productos">Productos</Link><Link to="/admin/clientes">Clientes</Link><button type="button" onClick={logoutAdmin}>Cerrar sesión</button></nav></header><div className="modeler-workspace">
    ...
    <aside className="modeler-properties"><p className="modeler-eyebrow">Propiedades</p><h2>{title}</h2>{tool==='wall'&&!selectedItem&&<><label>Altura <span><input type="number" min=".1" step=".1" value={settings.wallHeight} onChange={(e)=>setSettings({...settings,wallHeight:e.target.value})}/> m</span></label><label>Espesor <span><input type="number" min=".05" step=".01" value={settings.wallThickness} onChange={(e)=>setSettings({...settings,wallThickness:e.target.value})}/> m</span></label></>}{selection?.collection==='walls'&&selectedItem&&<><label>Longitud <span><input type="number" min=".1" step=".05" value={Number(wallLength(selectedItem).toFixed(3))} onFocus={remember} onChange={(e)=>updateSelectedWall('length',e.target.value)}/> m</span></label><label>Altura <span><input type="number" min=".1" step=".1" value={selectedItem.height} onFocus={remember} onChange={(e)=>updateSelectedWall('height',e.target.value)}/> m</span></label><label>Espesor <span><input type="number" min=".01" step=".01" value={selectedItem.thickness} onFocus={remember} onChange={(e)=>updateSelectedWall('thickness',e.target.value)}/> m</span></label><p className="modeler-property-note">Arrastrá los círculos del muro para mover sus extremos sobre la rejilla.</p></>}{selection?.collection==='openings'&&selectedItem&&<><label>Ancho <span><input type="number" min=".2" step=".05" value={selectedItem.width} onFocus={remember} onChange={(e)=>updateSelectedOpening('width',e.target.value)}/> m</span></label><label>Alto <span><input type="number" min=".2" step=".05" value={selectedItem.height} onFocus={remember} onChange={(e)=>updateSelectedOpening('height',e.target.value)}/> m</span></label>{selectedItem.type==='window'&&<label>Antepecho <span><input type="number" min="0" step=".05" value={selectedItem.sill} onFocus={remember} onChange={(e)=>updateSelectedOpening('sill',e.target.value)}/> m</span></label>}{selectedItem.type==='door'&&<label>Apertura <select value={selectedItem.swing||'left-in'} onFocus={remember} onChange={(e)=>updateSelectedDoorSwing(e.target.value)}><option value="left-in">Izquierda · interior</option><option value="right-in">Derecha · interior</option><option value="left-out">Izquierda · exterior</option><option value="right-out">Derecha · exterior</option></select></label>}<p className="modeler-property-note">Arrastrá el círculo para mover la abertura sin sacarla del muro.</p></>}{(tool==='door'||tool==='window')&&!selectedItem&&<><label>Ancho <span><input type="number" min=".3" step=".1" value={settings.openingWidth} onChange={(e)=>setSettings({...settings,openingWidth:e.target.value})}/> m</span></label><label>Alto <span><input type="number" min=".3" step=".1" value={settings.openingHeight} onChange={(e)=>setSettings({...settings,openingHeight:e.target.value})}/> m</span></label>{tool==='window'&&<label>Antepecho <span><input type="number" min="0" step=".1" value={settings.sill} onChange={(e)=>setSettings({...settings,sill:e.target.value})}/> m</span></label>}</>}{tool==='furniture'&&!selectedItem&&<label>Tipo <select value={settings.furnitureType} onChange={(e)=>setSettings({...settings,furnitureType:e.target.value})}>{Object.entries(FURNITURE).map(([key,item])=><option key={key} value={key}>{item.label}</option>)}</select></label>}{selection?.collection==='furniture'&&selectedItem&&<><label>Ancho <span><input type="number" min=".1" step=".1" value={selectedItem.width} onFocus={remember} onChange={(e)=>updateSelectedFurniture('width',e.target.value)}/> m</span></label><label>Profundidad <span><input type="number" min=".1" step=".1" value={selectedItem.depth} onFocus={remember} onChange={(e)=>updateSelectedFurniture('depth',e.target.value)}/> m</span></label><label>Altura <span><input type="number" min=".1" step=".1" value={selectedItem.height} onFocus={remember} onChange={(e)=>updateSelectedFurniture('height',e.target.value)}/> m</span></label><label>Posición X <span><input type="number" step=".25" value={selectedItem.x} onFocus={remember} onChange={(e)=>updateSelectedFurniture('x',e.target.value)}/> m</span></label><label>Posición Y <span><input type="number" step=".25" value={selectedItem.y} onFocus={remember} onChange={(e)=>updateSelectedFurniture('y',e.target.value)}/> m</span></label><label>Rotación <span><input type="number" min="-360" max="360" step="5" value={Math.round((selectedItem.rotation||0)*180/Math.PI)} onFocus={remember} onChange={(e)=>updateSelectedFurniture('rotation',e.target.value)}/> °</span></label><button className="modeler-property-action" onClick={rotateSelected}>Rotar 90°</button></>}
      ...
      <section className="modeler-building"><h3>Construcción</h3>
        ...
        {model.building?.floor.enabled&&<><label><span>Espesor</span><em><input type="number" min=".01" step=".01" value={model.building.floor.thickness} onFocus={remember} onChange={(e)=>updateBuildingSetting('floor','thickness',e.target.value)}/> m</em></label><label><span>Mostrar</span><input type="checkbox" checked={model.building.floor.visible} onChange={(e)=>{remember();updateBuildingSetting('floor','visible',e.target.checked)}}/></label></>}
        <label><span>Cielorraso</span><input type="checkbox" checked={model.building?.ceiling.enabled||false} onChange={(e)=>{remember();updateBuildingSetting('ceiling','enabled',e.target.checked)}}/></label>
        {model.building?.ceiling.enabled&&<><label><span>Altura</span><em><input type="number" min=".2" step=".1" value={model.building.ceiling.height} onFocus={remember} onChange={(e)=>updateBuildingSetting('ceiling','height',e.target.value)}/> m</em></label><label><span>Mostrar</span><input type="checkbox" checked={model.building.ceiling.visible} onChange={(e)=>{remember();updateBuildingSetting('ceiling','visible',e.target.checked)}}/></label></>}
        <label><span>Techo plano</span><input type="checkbox" checked={model.building?.roof.enabled||false} onChange={(e)=>{remember();updateBuildingSetting('roof','enabled',e.target.checked)}}/></label>
        {model.building?.roof.enabled&&<><label><span>Espesor</span><em><input type="number" min=".01" step=".05" value={model.building.roof.thickness} onFocus={remember} onChange={(e)=>updateBuildingSetting('roof','thickness',e.target.value)}/> m</em></label><label><span>Alero</span><em><input type="number" min="0" step=".1" value={model.building.roof.overhang} onFocus={remember} onChange={(e)=>updateBuildingSetting('roof','overhang',e.target.value)}/> m</em></label><label><span>Mostrar</span><input type="checkbox" checked={model.building.roof.visible} onChange={(e)=>{remember();updateBuildingSetting('roof','visible',e.target.checked)}}/></label></>}
        <p>{detectedRooms.length} ambiente(s) · {totalFloorArea.toFixed(2)} m² de piso · rejilla cada 10 cm.</p>
      </section>
      ...
      {detectedRooms.length>0&&<div className="modeler-list"><p>Ambientes</p>{detectedRooms.map((room)=><button key={room.id} className={selection?.collection==='rooms'&&selection.id===room.id?'is-active':''} onClick={()=>{setSelection({collection:'rooms',id:room.id});setTool('select')}}><span>{room.name}</span><small>{room.area.toFixed(2)} m²</small></button>)}</div>}
    </aside></div></section>
}
