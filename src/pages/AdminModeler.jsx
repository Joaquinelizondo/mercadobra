...
export default function AdminModeler() {
  ...
  useEffect(() => {
    ...
    if(view==='top'&&model.building?.floor.enabled&&model.building.floor.visible){detectedRooms.forEach((room)=>{const a=project({x:room.x-room.width/2,y:room.y-room.depth/2});const b=project({x:room.x+room.width/2,y:room.y+room.depth/2});ctx.fillStyle='rgba(185,169,142,.48)';ctx.fillRect(Math.min(a.x,b.x),Math.min(a.y,b.y),Math.abs(b.x-a.x),Math.abs(b.y-a.y));ctx.fillStyle='#6f6454';ctx.font='700 11px sans-serif';ctx.textAlign='center';ctx.fillText(`${room.area.toFixed(2)} m²`,(a.x+b.x)/2,(a.y+b.y)/2)})}
